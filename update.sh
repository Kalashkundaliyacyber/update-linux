...
# Step 11: Remove orphaned packages
echo "Checking for orphaned packages..."
sudo deborphan | xargs sudo apt-get -y remove --purge
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 12: Rebuild the APT package database
echo "Rebuilding APT package database..."
sudo dpkg --configure -a
sudo apt-get check
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 13: Remove old kernel versions
echo "Removing old kernel versions..."
sudo apt autoremove --purge -y
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 14: Clear system logs
echo "Clearing old system logs..."
sudo find /var/log -type f -name "*.gz" -delete
sudo find /var/log -type f -name "*.1" -delete
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 15: Sync system time and timezone
echo "Syncing system time..."
sudo timedatectl set-ntp true
sudo systemctl restart systemd-timesyncd
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 16: Check disk health with SMART
echo "Checking disk health..."
sudo apt install -y smartmontools
sudo smartctl --all /dev/sda
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 17: Optimize system swappiness
echo "Optimizing system swappiness..."
sudo sysctl vm.swappiness=10
echo "vm.swappiness=10" | sudo tee -a /etc/sysctl.conf
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 18: Clean temporary files
...
# Step 20: Test network connectivity
...
# Step 23: Run security updates only
echo "Installing security updates..."
sudo unattended-upgrade
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 24: Reindex APT package cache
echo "Rebuilding APT cache..."
sudo rm -rf /var/lib/apt/lists/*
sudo apt update
echo "------------------------------------------------------------------------------------------------------------------------"

# Step 25: Check open ports
